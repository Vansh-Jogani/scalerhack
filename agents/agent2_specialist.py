...
class SpecialistAgent:
    ...
    async def run(self, agent1_report: dict) -> None:
        ...
        staging_lat = self.staging_lat if self.staging_lat is not None else base_lat
        staging_lon = self.staging_lon if self.staging_lon is not None else base_lon
        for i in range(num_drones):
            drone_id = f"swarm-{self.agent_id}-{i}"
            offset = (i - num_drones // 2) * 0.00015
            self.world_state.add_drone(drone_id, drone_type, staging_lat + offset, staging_lon + offset)
